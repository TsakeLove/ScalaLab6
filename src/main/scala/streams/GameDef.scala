...
trait GameDef {
   ...
   * the game.
   */
 //створює початковий блок для нашого моделювання, блок, розташований в початковій позиції
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    //вернет массив где объединено 4 массива 4 возможные движениыя
    def neighbors: List[(Block, Move)] =
      List((right, Right)) ::: List((left, Left)) ::: List((up, Up)) ::: List((down, Down))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    // проверяет не ышли ли за пределы
    def legalNeighbors: List[(Block, Move)] = {
      neighbors.filter(el => el._1.isLegal)
    }

    /**
     * Returns `true` if the block is standing.
     */
   // isStanding, який повідомляє нам, чи стоїть Block чи ні:
     ...
     */
   //isLegal на  Block, який повідомляє нам, знаходиться блок на місцевості чи поза нею
    def isLegal: Boolean =
